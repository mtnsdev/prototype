        ...
        <div key={team.id} className="bg-foreground/[0.03] border border-white/[0.04] rounded-xl p-4">
          ...
          <div className="flex items-center gap-1 mt-2 flex-wrap">
            ...
            {team.memberIds.length > 5 && (
              <span className="text-2xs text-muted-foreground">+{team.memberIds.length - 5}</span>
            )}
          </div>
          <button
            type="button"
            onClick={() => toast(`Manage members — ${team.name} (demo)`)}
            className="text-2xs text-[var(--muted-info-text)] underline-offset-2 hover:underline mt-2"
          >
            Manage members
          </button>
        </div>
...
function sourceIconBg(source: DataSource): string {
  if (source.source_type === DataSourceType.Email) return "bg-sky-500/10 text-sky-800";
  if (source.source_type === DataSourceType.GoogleDrivePersonal) return "bg-foreground/[0.06] text-foreground";
  ...
  return (
    <div className="space-y-3">
      ...
      {sources.map((source) => {
        ...
        return (
          <div key={source.id} className="bg-foreground/[0.03] border border-white/[0.04] rounded-xl p-4">
      ...
      <div className="bg-foreground/[0.03] border border-white/[0.04] rounded-xl p-4 mt-4">
        <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex items-center gap-2 bg-foreground/[0.03] rounded-lg px-3 py-2 border border-white/[0.04]">
          ...
          <button
            ...
            className="text-2xs text-[var(--muted-info-text)] underline-offset-2 hover:underline shrink-0"
          >
            Copy
          </button>
        </div>
    ...
    >
      <div className="shrink-0 flex items-center justify-between p-4 border-b border-border">
        ...
        <button
          type="button"
          onClick={onClose}
          className="p-2 rounded-lg text-muted-foreground/90 hover:text-foreground hover:bg-foreground/[0.06]"
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6">
        <div className="flex gap-1 bg-foreground/[0.03] rounded-lg p-0.5 mb-6">
          <button
            type="button"
            className={cn(
