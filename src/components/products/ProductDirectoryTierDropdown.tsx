...
export default function ProductDirectoryTierDropdown({ selectedTiers, onChange }: Props) {
  ...
  const summary =
    selectedTiers.length === 0 ? (
      ...
      <span className="truncate text-xs text-foreground">{`${selectedTiers.length} tiers`}</span>
    );
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        className={cn(
          "flex max-w-[160px] items-center gap-2 rounded-lg border px-3 py-1.5 text-left text-xs transition-colors",
          selectedTiers.length > 0
