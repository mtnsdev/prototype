...
import {
  productListingCompactTitleLinkClass,
  productListingMetaLineClass,
  productListingTitleMultilineClass,
} from "@/lib/productListingPrimitives";
    ...
    return (
      <div className={cn("rounded-lg border border-border bg-foreground/[0.04] p-3 flex items-center gap-3", accent)}>
        ...
        <div className="min-w-0 flex-1">
          <Link href={`/dashboard/products/${id}`} className={productListingCompactTitleLinkClass}>
            {searchQuery ? highlightSearch(product.name || "—", searchQuery) : (product.name || "—")}
          </Link>
          <p className={cn(productListingMetaLineClass, "truncate !mt-1 text-muted-foreground/75")}>{location}</p>
        </div>
        <span className="text-xs text-muted-foreground shrink-0">{CATEGORY_LABELS[product.category]}</span>
      </div>
    );
  }
      ...
      <div className="p-4 flex-1 flex flex-col gap-2">
        <div className="flex items-start justify-between gap-2">
          <Link
            ...
            className={cn(productListingTitleMultilineClass, "min-w-0 flex-1 hover:underline")}
          >
            {searchQuery ? highlightSearch(product.name || "—", searchQuery) : (product.name || "—")}
          </Link>
          <DropdownMenu>
        ...
        <p className={cn(productListingMetaLineClass, "!mt-1.5 text-sm text-muted-foreground/75")}>{location}</p>
        {product.description && (
          <p className="text-sm text-muted-foreground line-clamp-2">{product.description}</p>
        )}
        <div className="flex flex-wrap gap-1.5 mt-auto">
          {product.price_range && (
            <span className="text-xs text-muted-foreground" title={PRICE_RANGE_DISPLAY[product.price_range]}>
          ...
          <span className={cn("text-xs px-1.5 py-0.5 rounded border border-input", VERIFICATION_BADGES[ver]?.variant === "default" && "bg-[var(--muted-success-bg)] text-[var(--muted-success-text)] border-[var(--muted-success-border)]")}>
