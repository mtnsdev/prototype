...
export default function EmailIngestionPage() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden bg-[#06060a] text-[#F5F5F5]">
      <header className="shrink-0 flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between px-6 py-4 border-b border-[rgba(255,255,255,0.08)]">
        ...
        <div className="flex flex-wrap items-center gap-3 shrink-0">
          <div className="flex items-center gap-2 bg-white/[0.02] rounded-lg px-4 py-2 border border-white/[0.04]">
            ...
            <code className="text-sm text-gray-300 truncate max-w-[min(100vw-8rem,22rem)]">
        ...
        <div className="px-6 py-6 max-w-[1200px]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            <div className="bg-white/[0.02] border border-white/[0.04] rounded-xl p-4">
              <span className="text-[10px] text-gray-500 uppercase tracking-wider">Total Emails</span>
              <div className="text-2xl font-semibold text-white mt-1">{TOTAL_EMAILS_DEMO}</div>
            </div>
            <div className="bg-white/[0.02] border border-white/[0.04] rounded-xl p-4">
              <span className="text-[10px] text-gray-500 uppercase tracking-wider">Unprocessed</span>
              <div className="text-2xl font-semibold text-amber-400 mt-1">{unprocessedCount}</div>
            </div>
            <div className="bg-white/[0.02] border border-white/[0.04] rounded-xl p-4">
              <span className="text-[10px] text-gray-500 uppercase tracking-wider">Attachments Indexed</span>
          ...
          <div className="rounded-xl border border-white/[0.06] overflow-hidden bg-[#0e0e12]">
                ...
                <tbody>
                  {filtered.map((email) => (
                    <tr
                      ...
                      className={cn(
                        "border-b border-white/[0.04] hover:bg-white/[0.02] cursor-pointer",
                    ...
                    >
                      <td className="py-3 pl-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-white truncate max-w-[300px]">{email.subject}</span>
                          {email.attachments.length > 0 && (
                            <Paperclip className="w-3 h-3 text-gray-500 shrink-0" />
                          )}
                        </div>
                      </td>
                      <td className="py-3 text-sm text-gray-400">{email.senderName}</td>
                      <td className="py-3 text-sm text-gray-500">{formatTableDate(email.receivedAt)}</td>
            ...
            <p className="px-4 py-2.5 text-[10px] text-gray-600 border-t border-white/[0.04] flex items-start gap-2 leading-relaxed">
              ...
              <span>
                ...
                <Link href="/dashboard/knowledge-vault" className="text-gray-400 underline-offset-2 hover:underline">
                  Knowledge Vault
                </Link>{" "}
                and expand{" "}
                <span className="text-gray-400 font-medium">Access change log — vault &amp; email</span>.
              </span>
            </p>
          </div>
        </div>
      ...
      {selected && (
        <>
          ...
          />
          <aside className="fixed right-0 top-0 h-full w-full max-w-[440px] bg-[#0e0e12] border-l border-white/[0.06] z-50 overflow-y-auto p-6 shadow-2xl">
            <EmailDetailBody
              email={selected}
              isAdmin={isAdmin}
...
function EmailDetailBody({
  ...
  return (
    <>
        ...
        <div className="flex flex-wrap items-center gap-2 mt-1">
          ...
          {isAdmin && (
            <select
              ...
              className="text-xs bg-white/[0.04] border border-white/[0.06] rounded-lg px-2 py-1 text-gray-300 outline-none"
            >
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-1 mt-1">
          {email.tags.map((tag) => (
            <span
              key={tag}
              className="text-[10px] text-gray-500 bg-white/[0.04] px-2 py-0.5 rounded flex items-center gap-1"
            >
              {tag}
              <button
                type="button"
        ...
        {showAddTag && (
          <div className="flex items-center gap-1.5 mt-2">
            ...
            />
            <button type="button" onClick={onCancelTag} className="text-[10px] text-gray-600">
              Cancel
            </button>
          </div>
        )}
      ...
      <div className="mb-6">
        <span className="text-[10px] text-gray-600 uppercase tracking-wider">Email Body</span>
        <div className="mt-2 bg-white/[0.02] border border-white/[0.04] rounded-xl p-4 text-sm text-gray-300 leading-relaxed whitespace-pre-wrap">
          {email.bodyText}
        </div>
      </div>
      ...
      <div className="mt-6 pt-4 border-t border-white/[0.04]">
        ...
        {email.forwarder_departed ? (
          <div className="mt-2 mb-2 flex items-start gap-1.5 rounded-lg border border-amber-500/20 bg-amber-500/[0.06] px-3 py-2 text-[10px] text-amber-100/90">
