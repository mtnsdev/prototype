...
export default function DetailSidebar({ vic, className, onShowTravelProfiles }: Props) {
  ...
  return (
    <aside
      className={cn(
        "rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.03)] overflow-hidden",
    ...
    >
      ...
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-3 text-left lg:hidden border-b border-[rgba(255,255,255,0.06)]"
      >
        <span className="text-xs font-semibold uppercase tracking-wider text-[rgba(245,245,245,0.6)]">Quick info & alerts</span>
      ...
      <div className={cn("px-4 py-3 space-y-4", collapsed && "max-lg:hidden")}>
        ...
        <div>
          ...
          <ul className="space-y-1.5 text-sm text-[rgba(245,245,245,0.85)]">
            <li><span className="text-[rgba(245,245,245,0.5)]">Advisor:</span> {advisorName}</li>
            <li><span className="text-[rgba(245,245,245,0.5)]">Client since:</span> {clientSince}</li>
            <li>
              <span className="text-[rgba(245,245,245,0.5)]">Status:</span>{" "}
              <span className={cn(
                "capitalize",
                vic.relationship_status === "active" && "text-[var(--muted-success-text)]",
                vic.relationship_status === "inactive" && "text-[rgba(245,245,245,0.6)]",
        ...
        {(passportWarning || birthdaySoon) && (
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-wider text-[rgba(245,245,245,0.5)] mb-2">Alerts</h3>
        ...
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-wider text-[rgba(245,245,245,0.5)] mb-2">Recent activity</h3>
          {recentActivity.length === 0 ? (
            <p className="text-xs text-[rgba(245,245,245,0.5)]">No recent changes</p>
          ) : (
            <ul className="space-y-2 text-xs text-[rgba(245,245,245,0.75)]">
              {recentActivity.map((e, i) => (
                <li key={i} className="flex gap-1.5">
                  <Calendar size={12} className="shrink-0 mt-0.5 text-[rgba(245,245,245,0.4)]" />
