...
function DimensionBar({ label, low, high, value }: DimensionBarProps) {
  ...
  return (
    <div>
      <span className="mb-1.5 block text-[10px] text-[#6B6560]">{label}</span>
      <div className="relative h-[4px] rounded-full bg-[rgba(255,255,255,0.04)]">
        <div
          className="absolute top-1/2 h-2.5 w-2.5 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-[#08080c] bg-[#C9A96E]"
          style={{ left: `${clamped}%` }}
        />
      </div>
      <div className="mt-1 flex items-center justify-between">
        <span className="text-[8px] text-[#4A4540]">{low}</span>
        <span className="text-[8px] text-[#4A4540]">{high}</span>
      </div>
    </div>
  );
}
...
function MiniMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-[rgba(255,255,255,0.03)] bg-[rgba(255,255,255,0.02)] p-3">
      <span className="mb-1 block text-[9px] text-[#4A4540]">{label}</span>
      <span className="text-[16px] font-light text-[#F5F0EB]">{value}</span>
    </div>
  );
}
...
function InsightRow({
  ...
  return (
    <div className="flex items-start gap-3">
      <Icon className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#6B6560]" aria-hidden />
      <div>
        <span className="block text-[10px] text-[#4A4540]">{label}</span>
        <span className="text-[12px] text-[#9B9590]">{value}</span>
      </div>
    </div>
  );
}
...
export default function AcuityReportTab({ vic, status, isRunning, onRun, onRefresh }: Props) {
  ...
  if (status === "failed") {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <h3 className="text-[15px] font-light text-[#F5F0EB]">Acuity run failed</h3>
        <p className="mt-1 max-w-sm text-center text-[12px] text-[#6B6560]">
          We couldn&apos;t complete the analysis. Try again in a moment.
        </p>
        <button
          type="button"
          onClick={onRun}
          className="mt-5 flex items-center gap-2 rounded-lg border border-[rgba(201,169,110,0.18)] bg-[rgba(201,169,110,0.10)] px-4 py-2 text-[12px] text-[#C9A96E] transition-colors hover:bg-[rgba(201,169,110,0.16)]"
        >
  ...
  if (status === "not_run" && !isRunning) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <h3 className="text-[15px] font-light text-[#F5F0EB]">Acuity not yet run</h3>
        <p className="mt-1 max-w-sm text-center text-[12px] text-[#6B6560]">
        ...
        <span className="mt-2 text-[9px] text-[#4A4540]">Takes approximately 30 seconds</span>
      </div>
    );
  }
  ...
  if (status === "running" || isRunning) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <h3 className="text-[15px] font-light text-[#F5F0EB]">Analyzing {name}...</h3>
        <p className="mt-1 text-[12px] text-[#6B6560]">Processing booking history, communications, and travel patterns</p>
        <div className="mt-4 h-[2px] w-48 overflow-hidden rounded-full bg-[rgba(255,255,255,0.03)]">
          <div className="h-full rounded-full bg-[#C9A96E] animate-acuity-progress" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-y-auto">
      <div className="border-b border-[rgba(255,255,255,0.03)] px-6 py-5">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[16px] font-light text-[#F5F0EB]">Acuity Profile</h2>
          ...
          <button
            type="button"
            onClick={onRefresh}
            className="flex items-center gap-1.5 rounded-lg border border-[rgba(255,255,255,0.06)] px-2.5 py-1.5 text-[10px] text-[#6B6560] transition-colors hover:text-[#9B9590]"
          >
      ...
      <div className="border-b border-[rgba(255,255,255,0.03)] px-6 py-5">
        <span className="mb-2 block text-[9px] font-medium uppercase tracking-[0.08em] text-[#4A4540]">
          Executive Summary
        </span>
        <p className="text-[13px] leading-relaxed text-[#9B9590]">{summary}</p>
      </div>

      <div className="border-b border-[rgba(255,255,255,0.03)] px-6 py-5">
        <span className="mb-4 block text-[9px] font-medium uppercase tracking-[0.08em] text-[#4A4540]">
          Profile Dimensions
        </span>
      ...
      <div className="border-b border-[rgba(255,255,255,0.03)] px-6 py-5">
        <span className="mb-3 block text-[9px] font-medium uppercase tracking-[0.08em] text-[#4A4540]">
          Travel Preferences
        </span>
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          <div>
            <span className="mb-2 block text-[10px] text-[#6B6560]">Preferred Regions</span>
            <div className="space-y-1.5">
              {["Japan", "Southeast Asia", "French Riviera", "Maldives"].map((region) => (
                <div key={region} className="flex items-center gap-2">
                  <div className="h-1.5 w-1.5 rounded-full bg-[#C9A96E]" />
                  <span className="text-[11px] text-[#F5F0EB]">{region}</span>
                </div>
              ))}
            </div>
          </div>
          <div>
            <span className="mb-2 block text-[10px] text-[#6B6560]">Preferred Brands</span>
            <div className="space-y-1.5">
              {["Aman", "Four Seasons", "One&Only", "Cheval Blanc"].map((brand) => (
                <div key={brand} className="flex items-center gap-2">
                  <div className="h-1.5 w-1.5 rounded-full bg-[#C9A96E]" />
                  <span className="text-[11px] text-[#F5F0EB]">{brand}</span>
                </div>
              ))}
            </div>
          </div>
          <div>
            <span className="mb-2 block text-[10px] text-[#6B6560]">Travel Style</span>
            <div className="flex flex-wrap gap-1.5">
              {["Family", "Cultural immersion", "Fine dining", "Spa & wellness", "Private guides", "Photography"].map(
                (tag) => (
                  <span
                    key={tag}
                    className="rounded border border-[rgba(201,169,110,0.10)] bg-[rgba(201,169,110,0.06)] px-2 py-0.5 text-[10px] text-[#C9A96E]"
                  >
                    {tag}
                  </span>
                )
      ...
      <div className="border-b border-[rgba(255,255,255,0.03)] px-6 py-5">
        ...
        <div className="mt-4">
          <span className="mb-2 block text-[10px] text-[#6B6560]">Spend Allocation</span>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-[9px] text-[#6B6560]">
        ...
        <div className="flex h-16 items-end gap-1">
          {["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"].map((month, i) => (
            <div key={month} className="flex flex-1 flex-col items-center gap-1">
              ...
              />
              <span className="text-[8px] text-[#4A4540]">{month}</span>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2">
          {observations.map((obs, i) => (
            <div key={i} className="flex items-start gap-2 py-1.5">
              <Sparkles className="mt-0.5 h-3 w-3 shrink-0 text-[#C9A96E]" />
              <span className="text-[12px] leading-relaxed text-[#9B9590]">{obs}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-6 py-5">
        ...
        <div className="space-y-2">
          {travelHistory.map((trip, i) => (
            ...
            >
              <span className="w-16 text-[10px] text-[#4A4540]">{trip.date}</span>
              <div className="min-w-0 flex-1">
                <span className="text-[12px] text-[#F5F0EB]">{trip.destination}</span>
                <span className="block text-[10px] text-[#6B6560]">{trip.property}</span>
              </div>
              <span className="text-[10px] text-[#6B6560]">{trip.duration}</span>
              <span className="rounded bg-[rgba(255,255,255,0.03)] px-1.5 py-0.5 text-[9px] text-[#4A4540]">{trip.type}</span>
            </div>
          ))}
        </div>
      </div>
