...
const HOME: ShellCrumb = { label: "Home", href: BRIEFING_ROOM_PATH };
...
export function getDefaultShellCrumbs(pathname: string): ShellCrumb[] {
  ...
  if (path === "/dashboard" || path === BRIEFING_ROOM_PATH) {
    return [HOME];
  }

  const parts = path.split("/").filter(Boolean);

  if (parts[0] === "admin" && parts[1] === "briefing-room" && parts.length === 2) {
    return [HOME, { label: "Briefing Room CMS" }];
  }
  ...
  if (parts.length === 2 && parts[1] === "briefing-room") {
    return [HOME];
  }
